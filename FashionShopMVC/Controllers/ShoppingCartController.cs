...
using FashionShopMVC.Models.VnPay;
...
namespace FashionShopMVC.Controllers
{
    public class ShoppingCartController : Controller
    {
        ...
        public async Task<IActionResult> OrderSuccess()
        {
            ...
            string templatePath = Path.Combine(_webHostEnvironment.WebRootPath, "assets", "customer", "template", "order.html");
           ...
           /* Message message = new Message(new string[] { user.Email }, "Đặt hàng thành công", template);

            _emailAuthService.SendAuthEmail(message);*/

            EmailOrderSucessService.SendMail("Công Ty TNHH BLVCK", "Đặt hàng thành công", template, user.Email);
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00" && vnp_TransactionStatus == "00")
                    {
                        ...
                        if (itemOrder != null)
                        {
                            ...
                            _notyfService.Success("Thanh toán thành công", 5);
                            return RedirectToAction("OrderSuccess");
                        }
                    }
                    else
